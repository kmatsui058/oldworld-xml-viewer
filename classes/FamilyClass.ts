...
export default class FamilyClass {
  ...
  get ambitions(): Goal[] {
    return goals.Root.Entry.map((item) => {
      ...
      goal.familyClassWeights.find((test) => test.familyClass.zType === this.zType)
    )
  }

  get families(): Family[] {
    return families.Root.Entry.filter((family) => family.FamilyClass === this.entry.zType).map(
      (item) => {
