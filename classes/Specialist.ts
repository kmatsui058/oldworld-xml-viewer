import specialists from '~/assets/data/xml/specialist'
import texts from '~/assets/data/xml/text-infos'
export default class Specialist {
  readonly entry: XmlSpecialist.Entry
  constructor(zType: string | XmlSpecialist.ZType) {
    const targetEntry = specialists.Root.Entry.find((entry) => entry.zType === zType)
